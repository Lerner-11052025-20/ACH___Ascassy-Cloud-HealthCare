// User Management view — theme-aware staff cards + Role Permissions matrix.
...
const USERS = [
  {
    ...
    role: 'Admin',
    roleIcon: Crown,
    roleTone: 'bg-violet-50 text-violet-700 dark:bg-violet-500/15 dark:text-violet-300',
  ...
  },
  {
    ...
    role: 'Pharmacist',
    roleIcon: Shield,
    roleTone: 'bg-brand-50 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300',
  ...
  },
  {
    ...
    role: 'Cashier',
    roleIcon: User,
    roleTone: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300',
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div className="flex flex-col leading-tight">
            <span className="text-[11px] font-semibold text-fg">User Management</span>
            <span className="text-[10px] text-fg-subtle">3 active · role-based access</span>
          </div>
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {USERS.map((u) => {
            ...
            return (
              <div key={u.email} className="rounded-xl border border-line p-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex min-w-0 flex-col leading-tight">
                    <span className="truncate text-[10px] font-semibold text-fg">{u.name}</span>
                    <span className="truncate text-[9px] text-fg-subtle">{u.email}</span>
                  </div>
                </div>
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-md bg-emerald-50 px-1.5 py-0.5 text-[9px] font-semibold text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">
        ...
        <div className="overflow-hidden rounded-xl border border-line">
          <div className="bg-surface-muted/70 px-3 py-2 text-[10px] font-semibold text-fg">Role Permissions</div>
          <div className="grid grid-cols-12 gap-2 border-t border-line px-3 py-1.5 text-[9px] font-semibold uppercase tracking-wider text-fg-subtle">
          ...
          <div className="divide-y divide-line">
            {PERMISSIONS.map((p) => (
              <div key={p.label} className="grid grid-cols-12 items-center gap-2 px-3 py-1.5 text-[10px]">
                <span className="col-span-6 font-medium text-fg">{p.label}</span>
                {p.cells.map((c, i) => (
                  <span key={i} className="col-span-2 flex items-center justify-center">
                    {c === 'Y' ? (
                      ...
                      <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-line-strong" />
                    )}
                  </span>
                ))}
              </div>
